...
export function AddRealPi() {
  ...
  // Paste the single key the installer printed → fill the three fields.
  const applyKey = (text: string) => {
    setKeyText(text);
    const ep = decodeKey(text);
    if (ep) {
      setIp(ep.ip);
      setPort(ep.port);
      setToken(ep.token);
      ...
      setError("That key isn't valid. Copy it exactly from the Pi installer.");
    }
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={{ flex: 1, paddingHorizontal: 24, paddingTop: insets.top + 20 }}>
          ...
          <Text style={[type.subhead, { color: c.text.secondary, marginTop: 6, marginBottom: 28 }]}>
            Run the installer on the Pi and paste the key it prints. Phone and Pi must be on the same network.
          </Text>

          {field('Paste connection key', keyText, applyKey)}

          <Text style={[type.micro, { color: c.text.tertiary, marginTop: 2, marginBottom: 16 }]}>
