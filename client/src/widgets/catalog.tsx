// Widget catalog: every design the gallery offers. Previews render from
// clearly-labelled SAMPLE data (deterministic smooth shapes) — the gallery is a
// design catalogue, not live telemetry; the real widgets (Phase 4, WidgetKit)
// will read the Agent. Families follow WidgetKit: systemSmall / systemMedium
// (Home Screen) and accessoryCircular / accessoryRectangular / accessoryInline
// (Lock Screen, monochrome by design).
...
// ---- sample-data generators (previews only — labelled in the gallery) ----

const PREVIEW_BASE: Record<SeriesKey, { base: number; amp: number }> = {
  'cpu.temp_c': { base: 52, amp: 6 },
  'cpu.util_pct': { base: 22, amp: 16 },
  'cpu.freq_mhz': { base: 1800, amp: 500 },
  'mem.used_pct': { base: 42, amp: 8 },
  'mem.available_bytes': { base: 4.6e9, amp: 8e8 },
  'disk.used_pct': { base: 48, amp: 2 },
  'net.rx_bps': { base: 1.4e6, amp: 9e5 },
  'net.tx_bps': { base: 9e4, amp: 5e4 },
  'load.1m': { base: 0.9, amp: 0.5 },
  'sys.uptime_s': { base: 14 * 86_400, amp: 0 },
};

/** Deterministic smooth sample value for a preview (NOT telemetry). */
export function previewValue(key: SeriesKey, t: number): number {
  const { base, amp } = PREVIEW_BASE[key];
  const w1 = Math.sin(t / 480_000);
  const w2 = Math.sin(t / 1_730_000 + 2.1);
  return Math.max(0, base + amp * (0.6 * w1 + 0.4 * w2));
}

function metricValue(key: SeriesKey, t: number): number {
...
function spark(key: SeriesKey, points = 24) {
  ...
  return Array.from({ length: points + 1 }, (_, i) => ({
    t: from + i * step,
    v: previewValue(key, from + i * step),
  }));
}
